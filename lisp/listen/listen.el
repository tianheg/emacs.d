...
According to `listen-lighter-format', which see."
  (when-let* ((player listen-player)
   ...
   (let* ((elapsed (or (listen--elapsed listen-player) 0))
        (length (or (listen--length listen-player) elapsed))
             ...
             `((?a . ,(lambda ()
              ...
              (?t . ,(lambda ()
                    (if-let ((title (alist-get 'title metadata nil nil #'equal)))
                      ...
                      "")))
              (?e . ,(lambda ()
                    (propertize (listen-format-seconds elapsed)
                            'face 'listen-lighter-time)))
              (?r . ,(lambda ()
                    (propertize (concat "-" (listen-format-seconds remaining))
                            'face 'listen-lighter-time)))
              (?s . ,(lambda ()
...
(defun listen-mode--update (&rest _ignore)
  ...
  (when (and (listen-valid-player-p listen-player)
