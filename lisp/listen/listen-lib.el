...
(defun listen-valid-player-p (player)
  "Return non-nil if PLAYER looks like a Listen player struct.
Guards generic method calls from nil/invalid values."
  (and player
       (recordp player)
       (memq (type-of player)
             '(listen-player listen-player-mpv listen-player-vlc))))
